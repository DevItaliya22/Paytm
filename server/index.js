...
app.post("/signup", async (req, res) => {
  ...
  if (parsedData.success) {
    ...
    return res
      .status(200)
      .json({ message: "Invalid input from user", errors: parsedData.error });
  }
});
...
app.post("/givemoney", authenticateJwt, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      sender.password === receiver.password
    ) {
...
app.post("/updatePassword", authenticateJwt, async (req, res) => {
  ...
  try {
    ...
    if (!parsedData.success) {
      return res
        .status(400)
        .json({ message: "Invalid input", errors: parsedData.error });
    }
...
app.post("/addfriends", authenticateJwt, async (req, res) => {
  ...
  if (!success) {
    return res
      .status(400)
      .json({ message: "Invalid input", errors: friendsSchema.error });
  }
...
app.get("/requestReceived", authenticateJwt, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error fetching received requests" });
  }
});
...
app.post("/sendRequest", authenticateJwt, async (req, res) => {
  ...
  try {
    ...
    if (reqSender.email === reqReceiver.email) {
      return res
        .status(200)
        .json({ message: "Cannot send request to same user" });
    }
    const obj = {
      from: reqSender._id,
      to: reqReceiver._id,
