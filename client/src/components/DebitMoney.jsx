...
function DebitMoney() {
  ...
  const handleDebitMoney = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/debitmoney", data2, config);
      const updatedBalance = response.data.balance;
      setBalance(updatedBalance); 
      setAmount(0); 
      
    } catch (err) {
      console.log("Error debiting money:", err);
      alert("Error debiting money. Please try again.");
    }
  };
          ...
          {showAmountField && submittedPassword && (
            <div>
              ...
              <input
                ...
                style={{ width: "330px", padding: "10px", margin: "10px auto", backgroundColor: "#142a47", border: "none", borderRadius: "5px", color: "#1359a4", fontSize: "15px", transition: "0.3s ease" }}
              />
              <button onClick={handleDebitMoney} className="submit-btn" style={{ width: "350px", padding: "10px", margin: "30px auto", border: "none", borderRadius: "5px", backgroundColor: "#0b2447", color: "#fff", transition: "0.5s ease-in-out" }}>Debit Money</button>
            </div>
          )}
        </div>
      </div>
